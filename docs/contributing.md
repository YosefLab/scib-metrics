...
you need additional python packages to run tests and build the documentation.

With [hatch][], running one of the following commands will automatically resolve the environments for testing and
...
Without hatch, using `pip` (requires `pip>=25.1` for dependency-group support):
...
To enable the checks locally, install [pre-commit][] and run
...
but the single point of truth for CI jobs is the Hatch test matrix defined in `pyproject.toml`.
This means that local testing via hatch and remote testing on CI test against the same python versions and use the same environments.
...
See scanpy's [developer docs](https://scanpy.readthedocs.io/en/latest/dev/documentation.html) for more information on how to write your own.
