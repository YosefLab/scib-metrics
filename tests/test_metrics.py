...
def test_nmi_ari_cluster_labels_leiden_reproducibility():
    ...
    out1 = scib_metrics.nmi_ari_cluster_labels_leiden(X, labels, optimize_resolution=False, resolution=3.0, seed=1)
    out2 = scib_metrics.nmi_ari_cluster_labels_leiden(X, labels, optimize_resolution=False, resolution=3.0, seed=100)
    nmi1, ari1 = out1["nmi"], out1["ari"]
    nmi2, ari2 = out2["nmi"], out2["ari"]
    assert nmi1 != nmi2
    assert ari1 != ari2
